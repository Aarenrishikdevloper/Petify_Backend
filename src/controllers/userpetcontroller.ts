...
export const addUserPet = async (
 ...
 try {
  ...
  return res
   .status(200)
   .json({ message: "Pet Creation Sucessfully", data: newpet });
 } catch (error) {
  console.log(error);
...
export const updateUserpet = async (
 req: Request<{ id: string }, any, updatepetbody>,
 res: Response,
) => {
 try {
  ...
  const petupdate = await UserPetsModel.findByIdAndUpdate(petid, updatedBody, {
   new: true,
   runValidators: true,
  });
  ...
  return res
   .status(200)
   .json({ message: "User pet updated Sucessfully", data: petupdate });
 } catch (error) {
  console.error(error);
  return res.status(500).json({ message: "Something Went Wrong" });
 }
...
export const deletepet = async (
 ...
 try {
  ...
  return res
   .status(200)
   .json({ message: "Pet deletion sucessfull", data: pets });
 } catch (error) {
  console.error(error);
  return res.status(500).json({ message: "Something Went Wrong" });
 }
